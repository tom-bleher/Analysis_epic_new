...
if [ -z "$DETECTOR_PATH" ] || [ -z "$SIM_CMD" ]; then
...
# Function to execute commands with appropriate environment
execute_commands() {
    cd ${DETECTOR_PATH}
...
# Execute commands directly or via singularity based on environment
if [ "$INSIDE_SINGULARITY" = true ]; then
    echo "Executing commands directly (inside Singularity)"
    execute_commands
else
    echo "Executing commands via Singularity"
    singularity exec --containall \
        --bind ${BIND_PATHS} \
        ${SIF_PATH} /bin/bash -c "$(declare -f execute_commands); execute_commands"
fi
