...
void gun_particles(int n_events = 1e5, double E_start = 10.0, double E_end = 10.0, string out_fname="genParticles.hepmc") {
  ...
  for (events_parsed = 0; events_parsed < n_events; events_parsed++) {
    ...
    double p = sqrt( pow(E,2) - pow(electronMass,2) );
    ...
    GenParticlePtr p4_out = std::make_shared<GenParticle>( 
        FourVector(p*sin(theta2)*cos(phi2), p*sin(theta2)*sin(phi2), p*cos(theta2), E), id, 1);

    v3->add_particle_in( p4_in );
    v3->add_particle_out( p4_out );
